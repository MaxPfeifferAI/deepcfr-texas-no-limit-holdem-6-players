...
class RandomAgent:
    ...
    def choose_action(self, state):
        ...
        if not state.legal_actions:
            ...
            return pkrs.Action(pkrs.ActionEnum.Fold) # Attempt Fold as fallback

        # Select a random legal action type
        ...
        # Handle Raise action
            ...
            # Filter amounts to ensure they are <= remaining_stake_after_call and >= min_raise_increment
            ...
            if not possible_additional_amounts:
                 print(f"RandomAgent WARNING: No valid additional raise amounts found. Falling back to Call.")
                 if pkrs.ActionEnum.Call in state.legal_actions:
                     return pkrs.Action(pkrs.ActionEnum.Call)
                 else:
                     return pkrs.Action(pkrs.ActionEnum.Fold)
            ...
            # Ensure the chosen amount is positive and doesn't exceed stake (redundant check, but safe)
            additional_raise = max(min_raise_increment, min(additional_raise, remaining_stake_after_call))

            action = pkrs.Action(action_enum, additional_raise)

            # In strict mode, validate the action before returning (optional)
            if STRICT_CHECKING:
                ...
                if test_state.status != pkrs.StateStatus.Ok:
                    ...
                    print(f"RandomAgent STRICT CHECK FAILED: Invalid Raise({additional_raise}). Falling back to Call. Log: {log_file}")
                    if pkrs.ActionEnum.Call in state.legal_actions:
                        return pkrs.Action(pkrs.ActionEnum.Call)
                    else:
